...
public class NoisyPhysicalPlantMock extends PerfectPlantModel {

    protected Random r = new Random();
    public double noise= 0.5;

    public NoisyPhysicalPlantMock(double noise){
        super();
        this.noise = noise;
    }
    ...
    protected PlantSnapshot constructResult() {
        ...
        UReal noisyBoxTemp = new UReal(this.getT().add(new UReal(actualNoise, 0)).getX(), 0.204);//noise/Math.sqrt(3));
        UReal noisyHeaterTemp = new UReal(this.getTHeater().add(new UReal(actualNoise, 0)).getX(), 0.204);//noise/Math.sqrt(3));
